...
class LabelSmoothingLoss(nn.Module):
    ...
    def forward(self, logits: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
        ...
        return self.criterion(logits, true_dist)/ target.size(0)
...
def greedy_decode(
    ...
    for _ in range(max_len - 1):
        ...
        next_tensor = torch.ones(1, 1).type_as(src).fill_(next_word).to(device)
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Train Transformer for German-English translation"
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=512,
        help="Model dimensionality (default: 512)"
    )
    parser.add_argument(
        "--num_heads",
        type=int,
    ...
    parser.add_argument(
        ...
        type=int,
        default=2048,
        help="Feedforward network dimension (default: 2048)"
    )
    parser.add_argument(
        "--dropout",
        type=float,
    ...
    parser.add_argument(
        ...
        type=int,
        default=4000,
        help="Warmup steps for Noam scheduler (default: 4000)"
    )
    parser.add_argument(
        "--label_smoothing",
        type=float,
    ...
    parser.add_argument(
        ...
        help="Device to use (default: auto - uses GPU if available)"
    )
    parser.add_argument(
        "--checkpoint",
        type=str,
        default="checkpoint.pt",
        help="Path to save/load checkpoint (default: checkpoint.pt)"
    )
    parser.add_argument(
        "--wandb_project",
        type=str,
...
def run_training_experiment(args=None) -> None:
    ...
    # Scheduler - Using NoamScheduler from "Attention Is All You Need"
    ...
    # Log final BLEU to W&B and finish run
    if not args.no_wandb:
        wandb.log({"test_bleu": bleu})
        wandb.finish()
